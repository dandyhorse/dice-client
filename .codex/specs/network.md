...
- `Сдаться` в started-матче требует подтверждения, затем отправляет `MATCH_FORFEIT` и переводит всех на финальный экран с кнопкой реванша.
- `MATCH_STATE.finishReason` определяет доступность реванша: `EXIT` и `DISCONNECT` скрывают кнопку реванша; остальные завершения показывают её.
- `ROOM_LEAVE` из started/finished игры закрывает комнату для всех и удаляет её.
- Если игрок отключился от сервера в started-матче, сервер завершает матч победой оппонента, затем закрывает комнату. Клиент показывает WIN/LOSE, скрывает реванш и оставляет игрока на финальном экране до ручного `Выйти`.
