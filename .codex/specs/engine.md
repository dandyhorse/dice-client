...
- `renderer: THREE.WebGLRenderer` — antialias, `PCFShadowMap`, `pixelRatio = min(devicePixelRatio, 2)`
...
1. `createScene()` — фон + ambient (0.55) + directional (1.1) c shadow map 2048×2048
...
requestAnimationFrame → gameLoop:
  ...
  3. physicsWorld.step(1/60, dt, 3)   — фиксированный шаг + до 3 substeps
  4. dice.syncMeshes()                — позиция/кватернион из тела в mesh
...
- Стены: 4 видимые `CANNON.Body + THREE.Mesh` по периметру **внутренней** области `(TABLE_WIDTH - 2·WALL_INSET) × (TABLE_DEPTH - 2·WALL_INSET)`. Mesh цвета `0x5c4a3a` (тёплое дерево), `castShadow + receiveShadow`. Высота `WALL_HEIGHT`, толщина `WALL_THICKNESS`. `WALL_INSET` нужен чтобы стены попадали в frustum top-down камеры (при contain-fit без inset стены ровно на границе FOV) и оставлял зелёную "рамку" между стеной и кромкой стола.
...
| `DICE_COUNT` | 2 | Сколько костей спавнить |
...
- Directional `0xffffff @ 0.8` из `(0.001, 18, 0.001)` — **строго сверху (90°)**, отбрасывает тени. Микро-смещение по X/Z, чтобы shadow camera lookAt не совпал с up-вектором (иначе degenerate). Тень от стены падает ровно под её footprint — на стол боковой тени не попадает.
...
- Shadow camera (directional): квадрат `±20` (покрывает 16×9 стол с запасом), near 0.5, far 60, bias -0.0005
...
- `pixelRatio = min(devicePixelRatio, 2)` — не убиваем GPU на ретине
